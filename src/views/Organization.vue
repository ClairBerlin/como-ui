...
import { UserAddIcon, TrashIcon, ArrowCircleUpIcon, ArrowCircleDownIcon } from '@heroicons/vue/outline';
...
const memberships = computed(() => {
  const msObj = store.getters['jv/get']('Membership', `$[?(@.organization_name=="${org?.value?.orgName}")]`);
...
const removeMember = async () => {
  console.log(`Removing member with ID ${memberToRemoveId.value} from the organization with ID ${currentOrgId.value}`);
...
const addMember = async () => {
  ...
  let user = undefined;
  try {
    user = await store.dispatch('jv/get', ['users', { params: { 'filter[search]': searchUserEmail.value } }]);
  } catch (err) {
    console.log(err);
  ...
  if (typeof user === 'undefined' || !user || !userIds[0] || userIds?.length > 1) {
...
const changeRole = async (membership) => {
  ...
  try {
    ...
    await store.dispatch('jv/patch', [newMembership, { url: `memberships/${mid}/` }]);
...
const changeRoleTooltip = (role) => t(role === 'O' ? 'role.downgrade' : 'role.upgrade');
</script>

<template>
  <div class="border-b border-gray-200 bg-white">
    <header class="mx-auto max-w-screen-xl px-4 pt-12 pb-4" v-if="$route.meta.title">
      <div class="">
  ...
  <div v-else class="mx-auto mt-8 max-w-screen-xl px-4">
    ...
    <div class="text-black">
      <div class="flex items-center justify-end">
        <div class="flex">
          <div v-if="isOwner" class="gray-button m-2 font-semibold" @click="openDeleteOrgModal">
            <TrashIcon class="mr-2 h-4 w-4" />
            <span>{{ $t('org.delete') }}</span>
          </div>
          <div v-if="isOwner" class="gray-button m-2 mr-0 font-semibold" @click="showAddMemberModal = true">
          ...
          >
            <div class="form-control">
              <label class="label">
                <span class="label-text font-bold text-black">{{ $t('user.email') }}</span>
              </label>
              <input
                type="email"
      ...
      <div class="text-md overflow-hidden rounded-sm bg-white ring-1 ring-gray-300">
            ...
            >
              ...
              <td class="whitespace-nowrap px-2 py-2">
                <div class="flex flex-col sm:flex-row">
                  ...
                  >
                    <ArrowCircleDownIcon v-if="membership?.role === 'O'" class="h-6 w-6" />
