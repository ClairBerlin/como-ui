...
<template>
  ...
  <div v-else-if="hasSensors" class="text-md mt-8 overflow-hidden rounded-sm bg-white ring-1 ring-gray-300">
    <table class="min-w-full divide-y divide-gray-200">
      <thead class="bg-gray-50">
        <tr>
          ...
          <th scope="col" class="px-2 py-3 text-right text-xs font-medium tracking-wider text-gray-500 sm:px-6">
            {{ $t('node.sampleCount') }}
          </th>
        </tr>
      </thead>
  ...
  >
    <div class="flex">
      ...
      <div class="ml-3">
        {{ $t('node.noNodes') }} {{ ' ' }} {{ $t('contactSupportToGetSensors') }} {{ ' ' }}
        <a href="mailto:info@como-berlin.de" class="font-medium text-yellow-700 underline hover:text-yellow-600">
          info@como-berlin.de
        </a>
      </div>
    </div>
