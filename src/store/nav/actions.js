const actions = {
  ...
  async changeOrganization({ dispatch, commit, state, rootGetters }, targetOrgId) {
    if (targetOrgId !== undefined) {
      console.log(`Change context to organization with ID ${targetOrgId}.`);
      commit('START_ORG_LOADING');
      if (state.loadedOrgId !== undefined && state.loadedOrgId !== targetOrgId) {
        clearInventory(commit);
      }
      const orgPromise = dispatch('jv/get', `organizations/${targetOrgId}/`, {
        root: true,
      });
      const userPromise = dispatch('jv/get', `organizations/${targetOrgId}/users/`, {
        root: true,
      });
  ...
  async loadInvetory({ dispatch, commit, state }, targetOrgId) {
    if (targetOrgId !== undefined && state.loadedOrgId !== targetOrgId) {
      ...
      console.log(`Loading inventory for organization with ID ${targetOrgId}...`);
      const sitePromise = dispatch('jv/get', ['sites', { params: { 'filter[organization]': targetOrgId } }], {
        root: true,
      });
      const roomPromise = dispatch('jv/get', ['rooms', { params: { 'filter[organization]': targetOrgId } }], {
        root: true,
      });
      const sensorPromise = dispatch('jv/get', ['nodes', { params: { 'filter[organization]': targetOrgId } }], {
        root: true,
      });
      const instPromise = dispatch('jv/get', ['installations', { params: { 'filter[organization]': targetOrgId } }], {
        root: true,
      });
      ...
      console.log(`Done loading inventory for organization with ID ${targetOrgId}.`);
    }
  },
